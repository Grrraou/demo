    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <div>
                ...
                <h1 class="text-2xl font-bold text-gray-900 mt-1">Stock locations</h1>
            </div>
            @if (auth()->user()->canEditInventory())
                <a href="{{ route('inventory.stock-locations.create') }}" class="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700">New location</a>
            @endif
            <nav class="flex gap-2 text-sm text-gray-500">
                ...
                <span>/</span>
                <a href="{{ route('inventory.stock-locations.index') }}" class="text-indigo-600 font-medium">Locations</a>
                <span>/</span>
                        ...
                        <tr>
                            ...
                            @if (auth()->user()->canEditInventory())
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                    <a href="{{ route('inventory.stock-locations.edit', $location) }}" class="text-indigo-600 hover:text-indigo-800">Edit</a>
                                </td>
                            @endif
                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ auth()->user()->canEditInventory() ? 3 : 2 }}" class="px-6 py-8 text-center text-gray-500">No locations. Run the inventory seeder or use the API to add data.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
